...
class UNet(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, time_dim=256, context_dim=512):
        super().__init__()
        ...
        self.down1 = ResBlock(base_channels, base_channels, time_dim, context_dim)
        self.down2 = ResBlock(base_channels, base_channels * 2, time_dim, context_dim)
        self.down3 = ResBlock(base_channels * 2, base_channels * 4, time_dim, context_dim)
        ...
        self.mid1 = ResBlock(base_channels * 4, base_channels * 4, time_dim, context_dim)
        self.mid2 = ResBlock(base_channels * 4, base_channels * 4, time_dim, context_dim)

        self.upsample3 = nn.ConvTranspose2d(base_channels * 4, base_channels * 4, 4, stride=2, padding=1)
        self.up3 = ResBlock(base_channels * 4 + base_channels * 4, base_channels * 2, time_dim, context_dim)

        self.upsample2 = nn.ConvTranspose2d(base_channels * 2, base_channels * 2, 4, stride=2, padding=1)
        self.up2 = ResBlock(base_channels * 2 + base_channels * 2, base_channels, time_dim, context_dim)

        self.upsample1 = nn.ConvTranspose2d(base_channels, base_channels, 4, stride=2, padding=1)
        self.up1 = ResBlock(base_channels + base_channels, base_channels, time_dim, context_dim)
